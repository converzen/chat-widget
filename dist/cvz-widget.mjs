...
`;if(M){M.debounceRendering;M.debounceRendering=t=>{requestAnimationFrame(t);};}var Qh="cvz-widget-host",lo=class{hostElement=null;buildId="c18e719-2026-09-10T12:34:15.096Z";init(t){if(this.hostElement){console.warn("cvzWidget is already initialized.");return}console.log("init: Initializing cvzWidget...",this.buildId),console.log("CSS Length:",ao.length);let n=document.createElement("div");n.id=Qh,document.body.appendChild(n);let r=n.attachShadow({mode:"open"}),i=document.createElement("style");i.textContent=ao,r.appendChild(i);let o=document.createElement("div");o.id="cvz-root",r.appendChild(o),fr(Me(fu,{config:t}),o),this.hostElement=n;}hide(){if(!this.hostElement){console.warn("hide: cvzWidget is not initialized.");return}this.hostElement&&(this.hostElement.remove(),this.hostElement=null);}},mu=new lo;typeof window<"u"&&(window.cvzWidget=mu);var Rz=mu;export{Rz as default};