...
`;if(N){N.debounceRendering;N.debounceRendering=t=>{requestAnimationFrame(t);};}var Uc="cvz-widget-host",Cr=class{hostElement=null;init(t){if(this.hostElement){console.warn("cvzWidget is already initialized.");return}console.log("init: Initializing cvzWidget..."),console.log("CSS Length:",Sr.length);let n=document.createElement("div");n.id=Uc,document.body.appendChild(n);let r=n.attachShadow({mode:"open"}),i=document.createElement("style");i.textContent=Sr,r.appendChild(i);let o=document.createElement("div");o.id="cvz-root",r.appendChild(o),vn(ze(Ta,{config:t}),o),this.hostElement=n;}hide(){if(!this.hostElement){console.warn("destroy: cvzWidget is not initialized.");return}this.hostElement&&(this.hostElement.remove(),this.hostElement=null);}},Aa=new Cr;typeof window<"u"&&(window.cvzWidget=Aa);var jx=Aa;return jx;})();